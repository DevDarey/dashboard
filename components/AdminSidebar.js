            ...
            <div className='w-1/6 h-full bg-highlight flex flex-col xs:hidden 2xs:hidden sm:hidden md:hidden lg:w-1/4'>
                ...
                <div className='flex-1'>
                    {adminRoutes.map((route, index) => (
                        <Fragment key={index}>
                            {index === 2 && (
                                <a className={`nav-link cursor-pointer admin-user`}>
                            ...
                            <Link href={route.url}>
                                <a
                                    ...
                                    } ${!route.image && 'pl-12 text-sm'}`}>
                                    {route.image && <img src={route.image} alt={route.title} />}
                                    {route.title}
                                </a>
                            </Link>
                        </Fragment>
            ...
            {!showMenus && (
                <img
                    ...
                    className={`w-12 h-12 absolute z-10 top-2 right-2 ${
                        notice ? 'md:right-72' : ''
                    } lg:hidden xl:hidden cursor-pointer md:z-20`}
                    onClick={() => setShowMenus(true)}
                />
            )}
            <div className={`modal-left ${showMenus ? 'left-0' : '-left-full'}`}>
                <div className='absolute top-0 left-0 w-full h-full bg-primary-blue  flex flex-col z-30 md:w-2/4 lg:w-1/2'>
                    ...
                    <div className='flex-1 py-4'>
                        {adminRoutes.map((route, index) => (
                            <Link href={route.url} key={index}>
                                <a
                                    className={`w-full h-12 text-white tex-lg flex items-center px-5 ${
